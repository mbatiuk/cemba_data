...
Notes added on 07/03/2022 - hisat-3n BAM strandness conversion
Read mpileup doc first: http://www.htslib.org/doc/samtools-mpileup.html

For Hisat-3n PE mapping:
...
If the YZ tag is "+", the read is C to T conversion, I change the flag to forward mapping (OT/OB, R2)
no matter R1 or R2 by read.is_forward = True
If the YZ tag is "-", the read is G to A conversion, I change the flag to reverse mapping (CTOT/CTOB, R1)
...
def _convert_bam_strandness(in_bam_path, out_bam_path):
    with pysam.AlignmentFile(in_bam_path) as in_bam, pysam.AlignmentFile(
        ...
        for read in in_bam:
            if not read.has_tag("YZ"):
                raise ValueError(
                    "BAM read has no YZ tag. This function requires hisat-3n BAM files."
                )
            if _is_read_ct_conversion_hisat3n(read):
...
def _bam_to_allc_worker(
    ...
    if tabix:
        subprocess.run(shlex.split(f"tabix -f -b 2 -e 2 -s 1 {output_path}"), check=True)
...
def bam_to_allc(
    ...
    reference_fasta
        ...
        use samtools faidx to build .fai index first. Do not compress that file.
    output_path
        Path to 1 output ALLC file
    cpu
        Number of processes to use in parallel. DO NOT use cpu > 1 for single cell ALLC generation.
    ...
    convert_bam_strandness
        Convert CT conversion reads into forward strand, GA conversion reads into reverse strand.
        During call methylation, we use strandness to parse the mpileup result.
        Set this parameter to True if you are doing PE mapping with hisat-3n.
    ...
    if cpu > 1:
        raise NotImplementedError("Parallel bam-to-allc is not yet supported. Use cpu=1.")
    else:
