...
from .utilities import genome_region_chunks, parse_chrom_size
...
def tabix_allc(allc_path, reindex=False):
    ...
    if not is_gz_file(allc_path):
        raise ValueError(
            ...
            f'or use "tabix" directly to index your file.'
        )
...
def _merge_cg_strand(in_path, out_path):
    ...
    with open_allc(in_path) as allc, open_allc(out_path, "w") as out_allc:
        ...
        # flush final dangling site at end of file
        if prev_line is not None:
            if prev_line[2] == "-":
                prev_line = [prev_line[0], str(int(prev_line[1]) - 1), "+"] + prev_line[3:]
            out_allc.write("\t".join(prev_line) + "\n")
    return
...
def extract_allc(
    ...
    cov_cutoff
        Max cov filter for a single site in ALLC. Sites with cov > cov_cutoff will be skipped.
    tabix
