...
rule merge_lanes: #merge the lanes from the same cell_id and read_type, generating cell fastq
    input: # cell_id = uid-index_name
        fqs=lambda wildcards: df_lane.loc[(df_lane.uid==wildcards.uid) & \
        (df_lane.index_name==wildcards.index_name) & \
        (df_lane.read_type==wildcards.read_type)].fastq_path.iloc[0]
    ...
    run:
        fq_outdir=pathlib.Path(os.path.dirname(output.fq)).absolute()
        fq_outdir.mkdir(exist_ok=True, parents=True)
        if len(input.fqs) > 1:
...
rule cell_qc:
    ...
    run:
        ...
        for cell_id, row in unmapped_cells.iterrows():
            uid = row['UID']
            skipped_dir = pathlib.Path(outdir).absolute() / uid / 'fastq/skipped'
            skipped_dir.mkdir(exist_ok=True, parents=True)

            # move both R1 R2 to skipped files, it will not be included in Snakefile
            for read_type in ['R1', 'R2']:
                fastq_path = pathlib.Path(outdir).absolute() / uid / f'fastq/{cell_id}-{read_type}.fq.gz'
