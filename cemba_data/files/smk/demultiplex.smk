...
rule summary_demultiplex:
    ...
    run:
        ...
        stat_list = []
        for path in glob.glob(outdir + "/*/demultiplex.stats.txt"):
            uid = path.split('/')[-2]
...
rule merge_lanes:
    ...
    output:
        fq=temp(outdir+"/{uid}/{read_type}.fq.gz")

    params:
        outdir=lambda wildcards: outdir+f"/{wildcards.uid}"
...
rule run_demultiplex: #{prefixes}-{plates}-{multiplex_groups}-{primer_names}_{pns}_{lanes}_{read_types}_{suffixes}.fastq.gz
    ...
    run:
        ...
        # rename demultiplexed fastq files to final cell paths
        uids=wildcards.uid.split('-')
        input_fqs=glob.glob(outdir+f"/{wildcards.uid}/demultiplex/*-R*.fq.gz")
